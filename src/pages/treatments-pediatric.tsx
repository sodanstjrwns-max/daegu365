...
/* ============================================================
   ...
   - 4단계 무통마취 기본 적용 강조 (소아치과는 수면치료 미시행)
   ============================================================ */
...
const TREATMENTS = [
  {
    name: '유치 레진 (충치 치료)',
    price: '6~10만원',
    desc: '면 수에 따라 1면 6만 / 2면 8만 / 3면이상 10만',
    icon: 'fa-tooth',
  },
  {
    name: 'SS 크라운 (유구치)',
...
const COOPERATION_LEVELS = [
  ...
  },
  {
    level: 'LEVEL 3',
    title: '협조가 어려운 어린이',
    desc: '광범위 충치·발치·외상 등 시술 자체가 어려운 경우. 진료를 여러 차례로 나누어 아이의 속도에 맞춰 진행합니다.',
    method: ['분할 단계 진료', '4단계 무통마취', '부모 동반 안심 진료'],
    color: 'brown',
  },
]
...
const WHY_US = [
  ...
  },
  {
    icon: 'fa-layer-group',
    title: '협조도 3단계 맞춤 진료',
    desc: 'TSD → 웃음가스 → 분할 단계 진료까지. 아이의 협조도에 맞춰 단계별로 옵션을 제공합니다.',
  },
  {
    icon: 'fa-syringe',
    title: '4단계 무통마취 기본 적용',
    desc: '어른보다 더 무통마취가 중요합니다. 가글→도포→무통마취기→본마취까지 어린이에게도 추가 비용 없이 표준 적용.',
  },
  {
    icon: 'fa-route',
    title: '평생 관리 로드맵',
...
const DEFAULT_FAQS = [
  ...
  },
  {
    ...
    a: '대구365치과는 협조도 3단계 맞춤 시스템을 운영합니다. (1) 잘 협조: TSD + 4단계 무통마취 (2) 약간 두려움: 웃음가스(N2O) 보조 (3) 협조 어려움: 진료를 여러 차례로 나누어 아이 속도에 맞춰 진행. 부모님과 상담 후 함께 결정합니다.',
  },
  {
  ...
  {
    q: '아이가 충치가 너무 많은데 어떻게 치료하나요?',
    a: '다수 충치는 우선순위와 부위를 나누어 단계적으로 치료합니다. 4단계 무통마취와 웃음가스 보조로 아이의 부담을 줄이며, 한 회욨 진료량은 아이의 협조도와 컨디션에 맞춰 조절합니다.',
  },
  {
...
export const PediatricTreatmentPage = ({
  ...
  return (
    <>
      ...
      {(() => { const t = getTldr('pediatric'); return t ? <TldrBox label={t.label} summary={t.summary} bullets={t.bullets} cta={t.cta} reviewer={t.reviewer} /> : null })()}
        ...
        <div class="max-w-7xl mx-auto px-6 lg:px-12">
          <div class="grid lg:grid-cols-12 gap-16">
            ...
            <div class="lg:col-span-7 space-y-6 t-body text-lg">
              {/* PPT 모바일 슬라이드 306 — 줄바꿈·줄띄움 적용, 기존 볼드 유지 */}
              <p>
                대부분의 치과공포증은<br/>
                <b>어린 시절 첫 치과 방문에서 시작</b>됩니다.<br/>
                갑자기 통증이 있어 끌려간 첫 방문,<br/>
                영문도 모르고 누워서 받은 시술,<br/>
                도구 소리, 차가운 진료실<br/>
                — 그 한 번의 기억이 평생을 갑니다.
              </p>
              <p>
                대구365치과의 소아치과는<br/>
                <b>“오늘 충치 하나를 더 잡느냐”</b>가 아니라<br/>
                <b>“이 아이가 평생 치과를 어떻게 기억할 것인가”</b>를 우선합니다.<br/>
                협조도가 부족하면 다음 방문으로 미루는 여유를,<br/>
                협조가 잘 되면 적극적인 예방 처치를.
              </p>
              <p>
                대표원장 본인이<br/>
                <b>치과 진료실 문 앞에서 돌아섰던 어린 시절</b>이 있기에,<br/>
                어떤 두려움이 평생을 좌우하는지 가장 잘 압니다.<br/>
                그 기억을 다음 세대에는 물려주지 않겠다는 다짐이<br/>
                소아치과의 출발점입니다.
              </p>
            </div>
          </div>
      ...
      {/* 8. RELATED — 4단계 무통마취 (소아치과는 수면치료 미시행) */}
      <section class="py-24 relative text-ivory overflow-hidden" style="background:var(--brown-950);">
        <div class="max-w-5xl mx-auto px-6 lg:px-12 text-center">
          ...
          <h2 class="t-display mb-6" style="color:#fdfbf7; text-shadow:0 2px 12px rgba(0,0,0,0.4);">
            아이에게 더 중요한,<br/>
            <span style="color:#c9a876;">4단계 무통마취.</span>
          </h2>
          <p class="t-lead max-w-2xl mx-auto mb-10" style="color:#fdfbf7; opacity:0.9;">
            어른보다 통증에 예민한 아이일수록 마취가 중요합니다. 가글→도포→무통마취기→본마취까지, 추가 비용 없이 모든 진료에 표준 적용합니다.
          </p>
          <a href="/treatments/painless-anesthesia" class="btn-outline-ivory">4단계 무통마취 자세히 보기 <i class="fas fa-arrow-right ml-2"></i></a>
        </div>
      </section>
      ...
      <section class="relative py-24 lg:py-32 text-ivory overflow-hidden" style="background:var(--brown-950);">
        <div class="absolute inset-0 opacity-10" style="background-image:url('/r2/images/clinic/care-luxury-room.webp');background-size:cover;background-position:center;"></div>
      ...
      <DoctorProfileBlock
        ...
        credentials={['소아치과 전문의', '행동조절·놀이치료', '4단계 무통마취 진료']}
        treatmentLabel="소아치과"
      />
